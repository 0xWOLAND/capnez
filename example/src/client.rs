...
pub async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    tokio::task::LocalSet::new().run_until(async move {
        ...
        req_builder.set_name(&msg);
        
        let info_bytes = serde_json::to_vec(&Information { major: "Computer Science".to_string(), age: 25 })?;
        let mut info_list = req_builder.init_information(info_bytes.len() as u32);
        for (i, &byte) in info_bytes.iter().enumerate() { info_list.set(i as u32, byte); }

        let reply = request.send().promise.await?;
        println!("received: {}", reply.get()?.get_message()?.to_str()?);
        Ok(())
    }).await
}